  ...
  def consensus(event: Event, state: State): (State, Option[Message], Option[TriggerTimeout], Option[Event]) = {
    ...
    event match {
      ...
      case EventProposal(height, round, _, proposal, polRound, sender) => {
        ...
        if (checkEventValidity()) {
          val (newMessage, newState) = if (polRound >= state.lockedRound || proposal.get == state.lockedValue.get || state.lockedRound == -1) {
            val message = MessageVote(height, round, Some(util.md5HashString(proposal.get)), Prevote, state.validatorID)
            ...
            val message = MessageVote(height, round, None, Prevote, state.validatorID)
